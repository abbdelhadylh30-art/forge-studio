  ...
  return (
    <div className="flex h-screen flex-col bg-slate-100 overflow-hidden">
      ...
      <div className="relative flex min-h-0 flex-1">
        <aside className={cn("hidden md:flex shrink-0 overflow-hidden border-r border-slate-200 bg-white transition-all duration-300 ease-out", libraryOpen ? "w-64" : "w-0")} aria-hidden={!libraryOpen}>
          <div className="h-full w-64"><SectionLibrary /></div>
        </aside>
        <main className="min-w-0 flex-1 overflow-hidden"><BuilderCanvas /></main>
        <aside className={cn("hidden md:flex shrink-0 overflow-hidden border-l border-slate-200 bg-white transition-all duration-300 ease-out", inspectorOpen ? "w-80" : "w-0")} aria-hidden={!inspectorOpen}>
          <div className="h-full w-80"><BuilderInspector /></div>
        </aside>
        {libraryOpen && (
          <>
            <div className="md:hidden fixed inset-0 z-50 bg-black/40 backdrop-blur-sm" onClick={() => setLibraryOpen(false)} aria-hidden="true" />
        ...
        {inspectorOpen && (
          <>
            <div className="md:hidden fixed inset-0 z-50 bg-black/40 backdrop-blur-sm" onClick={() => setInspectorOpen(false)} aria-hidden="true" />
