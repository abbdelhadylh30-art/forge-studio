...
- ✅ **P6 hero carousel — SHIPPED in v1.9** (hero.images extra slides turn the visual into an auto-rotating carousel: fade/slide/zoom/flip transitions via [data-lf-anim], 0–15s interval, arrows + dots, per-slide editor with AI image fields; one .lf-cs/.lf-cs-on class contract + vanilla export script so the standalone HTML animates identically; prefers-reduced-motion safe; YAML round-trip + guards)
- ✅ **P7 small settings — SHIPPED in v1.9** (brand.faviconUrl + seo.keywords fields with studio editors, published-page metadata icons/keywords + export `<link rel=icon>`/`<meta name=keywords>`; contact successMessage + redirectUrl with post-submit navigation on the published page and webhook exports; version snapshots — 5 named save-points in localStorage per project, camera button next to undo/redo, restore flows through setConfig so ⌘Z reverts it; share-link intentionally skipped — publish/deploy covers it)

**Backlog complete — P1 through P7 all shipped.** Remaining known limitations are deployment-level, not feature-level: durable serverless data needs Turso/Postgres in DATABASE_URL (per-instance SQLite is ephemeral on Vercel).
